...
/* The DynaHack client lib may be freely redistributed under the terms of either:
 *  - the NetHack license
			 ...
			 int gend, int align, enum nh_game_modes playmode)
{
    ...
    if (json_unpack(jmsg, "{si,si!}", "return", &ret, "gameid", &current_game) == -1) {
...
int nhnet_command(const char *cmd, int rep, struct nh_cmd_arg *arg)
{
    int ret;
    json_t *jmsg, *jarg;
	...
	case CMD_ARG_POS:
	    jarg = json_pack("{si,si,si}", "argtype", arg->argtype, "x",
			     arg->pos.x, "y", arg->pos.y);
	    break;
	    
	case CMD_ARG_OBJ:
	    jarg = json_pack("{si,si}", "argtype", arg->argtype, "invlet",
			     arg->invlet);
	    break;
    ...
    jmsg = json_pack("{ss,so,si}", "command", cmd ? cmd : "", "arg", jarg, "count", rep);
...
static struct nh_symdef *read_symdef_array(json_t *jarr)
{
    json_t *jobj;
...
static struct nh_listitem *read_json_list(json_t *jarr)
{
...
static void free_option_data(struct nh_option_desc *opt)
{
    int i;
...
struct nh_option_desc *nhnet_get_options(enum nh_option_list list)
{
	...
	count = json_array_size(jarr);
	option_lists[list] = olist = malloc(sizeof(struct nh_option_desc) * (count + 1));
...
static const char **read_string_array(json_t *jarr)
{
    json_t *jstr;
    const char **array;
    const char *str;
...
char *nhnet_build_plselection_prompt(char *buf, int buflen, int rolenum,
				     int racenum, int gendnum, int alignnum)
{
    ...
    if (!nhnet_active())
	return nh_build_plselection_prompt(buf, buflen, rolenum,
	...
	print_error("Incorrect return object in nhnet_build_plselection_prompt");
	ret = NULL;
    } else {
