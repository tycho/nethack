...
static const char *switch_windowprocs(const char *key)
{
...
void handle_display_list(json_t *display_list)
{
    int i, j, count;
    json_t *jwrap, *jobj;
    void *iter;
...
static json_t *cmd_update_status(json_t *params, int display_only)
{
	...
	for (i = 0; i < count; i++)
	    strcpy(player.statusitems[i], json_string_value(json_array_get(p, i)));
    }
...
static json_t *cmd_update_screen(json_t *params, int display_only)
{
    ...
    int ux, uy;
    int x, y, effect, bg, trap, obj, obj_mn, obj_stacks,
	...
	for (y = 0; y < ROWNO; y++) {
	    ...
	    if (json_unpack(elem, "[i,i,i,i,i,i,i,i,i,i!]",
			    &effect, &bg, &trap, &obj, &obj_mn, &obj_stacks,
	    ...
	    dbuf[y][x].obj_stacks = obj_stacks;
	    dbuf[y][x].mon = mon;
...
static void json_read_menuitem(json_t *jobj, struct nh_menuitem *mi)
{
    int accel, group_accel, selected;
    const char *caption;
    if (json_unpack(jobj, "{ss,si,si,si,si,si!}", "caption", &caption,
		    "id", &mi->id, "role", &mi->role, "accel", &accel,
		    "group_accel", &group_accel, "selected", &selected) == -1)
...
static void json_read_objitem(json_t *jobj, struct nh_objitem *oi)
{
    const char *caption;
...
static json_t *cmd_display_objects(json_t *params, int display_only)
{
    ...
    jarr = json_array();
    for (i = 0; i < ret; i++) {
	jobj = json_pack("{si,si}", "id", pick_list[i].id, "count", pick_list[i].count);
...
static json_t *cmd_query_key(json_t *params, int display_only)
{
    ...
    if (display_only) {
	print_error("cmd_query_key called when a return value was not expected.");
	return NULL;
    }
    
    if (json_unpack(params, "{ss,sb!}", "query", &query, "allow_count",
	            &allow_count) == -1) {
...
static json_t *cmd_getpos(json_t *params, int display_only)
{
    ...
    if (json_unpack(params, "{ss,si,si,si!}", "goal", &goal, "force", &force,
	            "x", &x, "y", &y) == -1) {
...
static json_t *cmd_getdir(json_t *params, int display_only)
{
    ...
    if (json_unpack(params, "{ss,si!}", "query", &query, "restricted", &restr) == -1) {
...
static json_t *cmd_yn_function(json_t *params, int display_only)
{
    ...
    if (display_only) {
	print_error("cmd_yn_function called when a return value was not expected.");
	return NULL;
    }
    
    if (json_unpack(params, "{ss,ss,si!}", "query", &query, "set", &set, "def", &def) == -1) {
