...
bool obj_is_pname(struct obj *obj)
{
 ...
 * or doname() instead.
 */
char *distant_name(struct obj *obj, char *FDECL((*func), (struct obj*)))
{
	char *str;
...
ring:
	...
	if ((obj->otyp != BOULDER) || !throws_rocks (youmonst.data))
		if ((obj->otyp < LUCKSTONE) && (obj->otyp != CHEST) && (obj->otyp != LARGE_BOX) &&
		    (obj->otyp != ICE_BOX) && flags.invweight)
 ...
 */
const char *singular(struct obj *otmp, char *FDECL((*func), (struct obj*)))
{
	long savequan;
#ifdef SHOW_WEIGHT
	unsigned saveowt;
 ...
 */
struct obj *readobjnam(char *bp, struct obj *no_wish, bool from_user)
{
	char *p;
	int i;
	struct obj *otmp;
