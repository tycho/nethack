 ...
 */
bool goodpos(int x, int y, struct monst *mtmp, unsigned gpflags)
{
...
bool enexto_core(coord *cc, xchar xx, xchar yy, struct permonst *mdat,
		unsigned entflags)
{
#define MAX_GOOD 15
 ...
 */
STATIC_OVL bool tele_jump_ok(int x1, int y1, int x2, int y2)
{
	if (dndest.nlx > 0) {
...
STATIC_OVL bool teleok(int x, int y, bool trapok)
{
...
void teleds(int nux, int nuy, bool allow_drag)
{
...
bool safe_teleds(bool allow_drag)
{
	int nux, nuy, tcnt = 0;
	...
	do {
		nux = rnd(COLNO-1);
		nuy = rn2(ROWNO);
	} while (!teleok(nux, nuy, (bool)(tcnt > 200)) && ++tcnt <= 400);
...
STATIC_OVL void vault_tele(void)
{
...
bool teleport_pet(struct monst *mtmp, bool force_it)
{
	struct obj *otmp;
...
int mlevel_tele_trap(struct monst *mtmp, struct trap *trap, bool force_it, int in_sight)
{
	int tt = trap->ttyp;
...
bool u_teleport_mon(struct monst *mtmp, bool give_feedback)
{
	coord cc;
