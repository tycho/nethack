...
bot2()
{
    ...
    // TODO(skishore): Why does encglyph escaping screw up with status colors?
    //Sprintf(nb = eos(newbot2), "%s:%-2ld",
    //        encglyph(objnum_to_glyph(GOLD_PIECE)), money_cnt(invent));
    Sprintf(nb = eos(newbot2), "%s:%-2ld", "$", money_cnt(invent));
