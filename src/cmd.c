 ...
 */
void set_occupation(int NDECL ((*fn)), const char *txt, int xtime)
{
	if (xtime) {
...
STATIC_PTR int doextcmd(void)	/* here after # - now read a full-word command */
{
	int idx, retval;
...
STATIC_PTR int wiz_wish(void)	/* Unlimited wishes for debug mode by Paul Polderman */
{
	if (wizard) {
...
#ifdef SHOW_BORN
/* #showkills command */
STATIC_PTR int wiz_showkills(void)		/* showborn patch */
{
 ...
 */
STATIC_OVL bool minimal_enlightenment(void)
{
	winid tmpwin;
	menu_item *selected;
	anything any;
...
STATIC_OVL void count_obj(struct obj *chain, long *total_count,
		long *total_size, bool top, bool recurse)
{
	long count, size;
	struct obj *obj;
...
STATIC_OVL void obj_chain(winid win, const char *src, struct obj *chain,
...
STATIC_OVL void mon_invent_chain(winid win, const char *src, struct monst *chain,
		long *total_count, long *total_size)
{
	char buf[BUFSZ];
	long count = 0, size = 0;
...
STATIC_OVL void mon_chain(winid win, const char *src, struct monst *chain,
		long *total_count, long *total_size)
{
	char buf[BUFSZ];
	long count, size;
