...
struct obj *o_in(struct obj *obj, int oclass)
{
    struct obj* otmp;
    struct obj *temp;
...
STATIC_OVL bool check_map_spot(int x, int y, int oclass, unsigned material)
{
	int glyph;
	struct obj *otmp;
	struct monst *mtmp;
...
/*
   ...
   change occurs.
 */
STATIC_OVL bool clear_stale_map(int oclass, unsigned material)
{
	int zx, zy;
