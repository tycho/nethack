...
int parse_ip_addr(const char *str, struct sockaddr *out, int want_v4)
{
    ...
    memcpy(out, gai_res->ai_addr,
	   want_v4 ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6));
    do {
...
{
    if (settings.logfile) free(settings.logfile);
    if (settings.pidfile) free(settings.pidfile);
    if (settings.workdir) free(settings.workdir);
    if (settings.dbhost) free(settings.dbhost);
    if (settings.dbport) free(settings.dbport);
    if (settings.dbname) free(settings.dbname);
    if (settings.dbuser) free(settings.dbuser);
    if (settings.dbpass) free(settings.dbpass);
