...
void amulet(void)
{
	...
	for (mtmp = level->monlist; mtmp; mtmp = mtmp->nmon)
		...
		if (distu(mtmp->mx,mtmp->my) > 2)
		    pline("get the creepy feeling that somebody noticed "
		          "your taking the Amulet.");
		return;
	    }
}
...
static long target_on(int mask, struct monst *mtmp)
{
	...
	if (!mon_has_arti(mtmp, otyp)) {
		...
		if ((mtmp->data->geno & G_UNIQ) && (mtmp2->data->geno & G_UNIQ))
		    return STRAT_NONE;
		else
...
int tactics(struct monst *mtmp)
{
	...
	if (mtmp->data == &mons[PM_CTHULHU] && strat == STRAT_HEAL)
	    strat = STRAT_NONE;
