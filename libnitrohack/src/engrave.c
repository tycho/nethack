...
void wipe_engr_at(struct level *lev, xchar x, xchar y, xchar cnt)
{
	...
	if (ep && ep->engr_type != HEADSTONE){
		...
		while (ep->engr_txt[0] == ' ')
			ep->engr_txt++;
		if (!ep->engr_txt[0]) del_engr(lev, ep);
	    }
	}
}
...
static int doengrave_core(struct obj *otmp, boolean auto_elbereth)
{
	...
	const char *eground;	/* Type of ground (i.e. dust/floor/...) */
	...
	switch (type) {
		...
		eground = (is_ice(level, u.ux, u.uy) ? "frost" : "dust");
		break;
	    case HEADSTONE:
	    ...
	    case BURN:
		everb = (oep && !eow ? "add" :
			 (is_ice(level, u.ux, u.uy) ? "melt" : "burn"));
		eloc = (oep && !eow ?
			(is_ice(level, u.ux, u.uy) ?
	...
	if (u.roleplay.illiterate) {
	    sprintf(ebuf, "X");
	} else if (auto_elbereth) {
	    strcpy(ebuf, "Elbereth");
	} else {
	    sprintf(qbuf, "What do you want to %s %s the %s here?",
		    everb, eloc, eground);
	    getlin(qbuf, ebuf);
	}
...
{
	return doengrave_core(o, FALSE);
}

int doelbereth(void)
{
	return doengrave_core(NULL, TRUE);
}
