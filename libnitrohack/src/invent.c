...
void addinv_core1(struct obj *obj)
{
	...
	if (obj->otyp == AMULET_OF_YENDOR) {
		...
		set_artifact_intrinsic(obj, 1, W_ART, TRUE);
	}
}
...
void freeinv_core(struct obj *obj)
{
	...
	if (obj->oclass == COIN_CLASS) {
		...
		set_artifact_intrinsic(obj, 0, W_ART, TRUE);
	}
...
{
	if (Hallucination) {
		int c = display_rng(SIZE(currencies));
...
int doprwep(void)
{
	...
	prinv(NULL, uwep, 0L);
	if (u.twoweap) prinv(NULL, uswapwep, 0L);
    }
    return 0;
}
...
{
	if (invbuf) free(invbuf),  invbuf = NULL;
	invbufsiz = 0;
}
