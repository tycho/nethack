...
static void clear_unpaid(struct monst *shkp, struct obj *list)
{
    while (list) {
	if (Has_contents(list)) clear_unpaid(shkp, list->cobj);
	if (onbill(list, shkp, TRUE)) list->unpaid = 0;
	list = list->nobj;
    }
}
...
static int dopayobj(struct monst *shkp,
		    struct bill_x *bp,
		    struct obj **obj_p,
...
static void add_to_billobjs(struct obj *obj)
{
	...
	panic("add_to_billobjs: obj not free (%d,%d,%d)",
...
static long stolen_container(struct obj *obj, struct monst *shkp,
			     long price, boolean ininv)
{
	struct obj *otmp;
...
long stolen_value(struct obj *obj, xchar x, xchar y, boolean peaceful,
		  boolean silent)
{
...
static void makekops(coord *mm)
{
	...
	mc = malloc(cnt * sizeof(coord));
	for (ik = 0; kop_pm[ik]; ik++) {
	    if (!(mvitals[kop_pm[ik]].mvflags & G_GONE)) {
		cnt = epathto(mc, kop_cnt[ik], level, mm->x, mm->y, &mons[kop_pm[ik]]);
		while(--cnt >= 0)
		    makemon(&mons[kop_pm[ik]],
			    level, mc[cnt].x, mc[cnt].y, NO_MM_FLAGS);
	    }
	}
	free(mc);
}
