...
int m_move(struct monst *mtmp, int after)
{
	...
	if (is_swamp(level, mtmp->mx, mtmp->my) && rn2(3) &&
...
#define SQSRCHRADIUS	5
	...
	if ((passes_bars(ptr) || metallivorous(ptr)) && !In_sokoban(&u.uz))
	    flag |= ALLOW_BARS;
...
postmov:
	if (mmoved == 1 || mmoved == 3) {
		...
		/* possibly eat through iron bars */
		if (level->locations[mtmp->mx][mtmp->my].typ == IRONBARS) {
		    if (dmgtype(ptr, AD_DISN)) {
			if (canseeit)
			    pline("%s dissolves the iron bars.", Monnam(mtmp));
		    ...
		    } else if (flags.verbose && canseemon(level, mtmp)) {
		...
		if (gold_at(level, mtmp->mx,mtmp->my) && likegold) mpickgold(mtmp);
...
boolean closed_door(struct level *lev, int x, int y)
{
...
{
	return (boolean)(ACCESSIBLE(level->locations[x][y].typ) && !closed_door(level, x, y));
}
