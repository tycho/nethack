...
static int kick_object(xchar x, xchar y, schar dx, schar dy)
{
	...
	if (Is_box(kickobj)) {
		...
		/* can't kick these open or break them */
		if (kickobj->otyp == IRON_SAFE) return 1;
	...
	if (mon) {
	    ...
	    if (isgold ? ghitm(mon, kickobj) :			/* caught? */
			 thitmonst(mon, kickobj, kickstack))	/* hit && used up? */
		return 1;
	}
...
dumb:
	...
	/* Ali - artifact doors */
	if (artifact_door(level, x, y)) goto ouch;
