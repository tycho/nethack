...
static void spo_object(struct sp_coder *coder, struct level *lev)
{
	...
	while (nparams++ < SP_O_V_END + 1 &&
		...
		if ((OV_typ(parm) == SPOVAR_MONST)) {
			...
			if (def_char_to_monclass(monclass) != MAXMCLASSES) {
			    pm = mkclass(&lev->z, def_char_to_monclass(monclass),
					 G_NOGEN);
			} else {
			    pm = rndmonst(lev);
			}
	...
	do {
	    ...
	    quancnt--;
		 ...
		 (tmpobj.id > STRANGE_OBJECT && !objects[tmpobj.id].oc_merge));
...
static void spo_wallify(struct sp_coder *coder, struct level *lev)
{
	...
	wallify_map(lev, dx1 < 0 ? xstart : dx1,
			 dy1 < 0 ? ystart : dy1,
	...
	while (coder->frame->n_opcode < lvl->n_opcodes && !coder->exit_script) {
	    ...
	    case SPO_MATH_ADD:
		{
			...
			if (OV_typ(a) == SPOVAR_INT) {
			    ...
			    char *tmpbuf = malloc(strlen(OV_s(a)) +
						  strlen(OV_s(b)) + 1);
			    if (tmpbuf) {
				struct opvar *c;
				sprintf(tmpbuf, "%s%s", OV_s(a), OV_s(b));
			    ...
			    splev_stack_push(coder->stack, a);
			    opvar_free(b);
			    impossible("adding unaddable types");
			}
		    } else {
			splev_stack_push(coder->stack, a);
			opvar_free(b);
