...
extern void scan_map(char *,sp_lev *,mazepart *);
...
cobj_ifstatement: IF_ID chance
		  {
			...
			if (n_if_list >= MAX_NESTED_IFS)
			    yyerror("IF-statements nested too deeply!");
			tmpcmp->cmp_what = 0;
			tmpcmp->cmp_val = (long) $2;
			add_opcode(&splev, SPO_CMP, tmpcmp);
...
monster_info	: ',' string
		  {
			monster *tmpm = (monster *)get_last_opcode_data1(&splev, SPO_MONSTER);
...
stair_region	: STAIR_ID ':' lev_region
		 ...
		 ',' lev_region ',' UP_OR_DOWN
		  {
			lev_region *tmplreg = (lev_region *)get_last_opcode_data1(&splev, SPO_LEVREGION);
