...
extern void add_opcode(sp_lev *,int,void *);
extern void *get_last_opcode_data1(sp_lev *,int);
extern void *get_last_opcode_data2(sp_lev *,int,int);
...
extern void check_vardef_type(struct lc_vardefs *,char *,long,char *);
...
static int in_switch_statement = 0;
		...
		| LEV_INIT_ID ':' SOLID_FILL_ID ',' terrain_type
		  {
			...
			add_opvars(splev, "iiii iiiio",
				   LVLINIT_SOLIDFILL, filling, 0, (long)$5.lit,
				   0, 0, 0, 0,
				   SPO_INITLEVEL);
			max_x_map = COLNO - 1;
			max_y_map = ROWNO;
...
shuffle_detail	: SHUFFLE_ID ':' VARSTRING
		  {
			...
			vd = vardef_defined(variable_definitions, $3, 1);
			if (vd) {
...
variable_define	: VARSTRING '=' INTEGER
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-integer variable "
					 "'%s' as integer", $1);
			    }
			} else {
		...
		| VARSTRING '=' STRING
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-string variable "
					 "'%s' as string", $1);
			    }
			} else {
		...
		| VARSTRING '=' VARSTRING
		  {
			...
			if (!strcmp($1, $3)) {
			    lc_error("Trying to set variable '%s' "
				     "to value of itself", $1);
			}
			vd2 = vardef_defined(variable_definitions, $3, 1);
			if (!vd2) {
			    lc_error("Trying to use an undefined variable '%s'",
				     $3);
			} else {
			    vd1 = vardef_defined(variable_definitions, $1, 1);
			    if (vd1) {
				if (vd1->var_type != vd2->var_type) {
				    lc_error("Trying to redefine non-string "
					     "variable '%s' as string", $1);
				}
			    } else {
		...
		| VARSTRING '=' TERRAIN_ID ':' mapchar
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-mapchar variable "
					 "'%s' as mapchar", $1);
			    }
			} else {
		...
		| VARSTRING '=' MONSTER_ID ':' encodemonster
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-monst variable "
					 "'%s' as monst", $1);
			    }
			} else {
		...
		| VARSTRING '=' OBJECT_ID ':' encodeobj
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-obj variable "
					 "'%s' as obj", $1);
			    }
			} else {
		...
		| VARSTRING '=' encodecoord
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-coord variable "
					 "'%s' as coord", $1);
			    }
			} else {
		...
		| VARSTRING '=' encoderegion
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine non-region variable "
					 "'%s' as region", $1);
			    }
			} else {
		...
		| VARSTRING '=' '{' integer_list '}'
		  {
			...
			if (vd) {
				...
				lc_error("Trying to redefine variable "
					 "'%s' as integer array", $1);
			    }
			} else {
...
function_define	: FUNCTION_ID NQSTRING '(' ')'
		  {
			...
			if (in_function_definition) {
			    lc_error("Recursively defined functions not allowed "
				     "(function %s).", $2);
			}
...
opt_percent	: /* nothing */
		...
		| PERCENT
		  {
			if ($1 < 0 || $1 > 100)
			    lc_error("Unexpected percentile chance '%li%%'", $1);
			$$ = $1;
		  }
		;
...
loopstatement	: LOOP_ID '[' integer_or_var ']'
		  {
			...
			if (n_if_list >= MAX_NESTED_IFS) {
			    lc_error("LOOP: Conditionals nested too deeply.");
...
ifstatement	: IF_ID comparestmt
		  {
			...
			if (n_if_list >= MAX_NESTED_IFS) {
			    lc_error("IF: Conditionals nested too deeply.");
			    n_if_list = MAX_NESTED_IFS - 1;
			}
...
room_pos	: '(' INTEGER ',' INTEGER ')'
		  {
			if ($2 < 1 || $2 > 5 ||
			    $4 < 1 || $4 > 5) {
			    lc_error("Room positions should be between 1-5: "
				     "(%li,%li)!", $2, $4);
			} else {
			    $$.x = $2;
			    $$.y = $4;
...
door_detail	: ROOMDOOR_ID ':' secret ',' door_state ',' door_wall ',' door_pos
		  {
			...
			if ($7 == ERR && $9 != ERR) {
			    lc_error("If the door wall is random, "
				     "so must be its pos!");
			} else {
			    add_opvars(splev, "iiiio",
				       (long)$9, (long)$5, (long)$3, (long)$7,
				       SPO_ROOM_DOOR);
			}
		  }
...
map_definition	: NOMAP_ID
		  {
			add_opvars(splev, "ciisiio", 0, 0, 1, NULL, 0, 0, SPO_MAP);
			max_x_map = COLNO - 1;
			max_y_map = ROWNO;
		  }
...
mon_generation	: MON_GENERATION_ID ':' SPERCENT ',' mon_gen_list
		  {
			...
			if (total_mons < 1)
			    lc_error("Monster generation: zero monsters defined?");
...
mon_gen_part	: '(' integer_or_var ',' monster ')'
		  {
			long token = $4;
			if (token == ERR)
			    lc_error("Monster generation: Invalid monster symbol");
		...
		| '(' integer_or_var ',' string ')'
		  {
			...
			if (token == ERR)
			    lc_error("Monster generation: Invalid monster name");
...
monster_detail	: MONSTER_ID chance ':' monster_desc
		  {
			...
			if ( 1 == $2 ) {
				...
				lc_error("Conditional creation of monster, "
...
object_detail	: OBJECT_ID chance ':' object_desc
		  {
			...
			if ( 1 == $2 ) {
				...
				lc_error("Conditional creation of obj, "
					 "but no jump point marker.");
			    }
			}
		  }
...
object_info	: CURSE_TYPE
		...
		| STRING
		  {
			long token = get_monster_id($1, '\0');
			if (token == ERR) {
			    /* "random" */
			    yywarning("OBJECT: Are you sure you didn't mean "
				      "NAME:\"foo\"?");
			    token = NON_PM - 1;
			}
...
trap_detail	: TRAP_ID chance ':' trap_name ',' coord_or_var
		  {
			...
			if ( 1 == $2 ) {
				...
				lc_error("Conditional creation of trap, "
					 "but no jump point marker.");
			    }
			}
		  }
...
stair_region	: STAIR_ID ':' lev_region ',' lev_region ',' UP_OR_DOWN
		  {
			add_opvars(splev, "iiiii iiiii iiso",
				   ...
				   (long)($7 ? LR_UPSTAIR : LR_DOWNSTAIR), 0, NULL,
				   SPO_LEVREGION);
		  }
		;
...
replace_terrain_detail : REPLACE_TERRAIN_ID ':' region ',' CHAR ',' terrain_type ',' SPERCENT
		  {
			...
			if (from_ter >= MAX_TYPE)
			    lc_error("Replace terrain: illegal map char '%c'", $5);
			...
			add_opvars(splev, "iiii iiiio",
				   $3.x1, $3.y1, $3.x2, $3.y2,
				   from_ter, to_ter, (long)$7.lit, chance,
				   SPO_REPLACETERRAIN);
		  }
		;
...
terrain_detail	: TERRAIN_ID chance ':' coord_or_var ',' terrain_type
		  {
			...
			c = $6.ter;
			if (c >= MAX_TYPE)
			    lc_error("Terrain: illegal map char");
			...
			if ( 1 == $2 ) {
				...
				lc_error("Conditional terrain modification, "
					 "but no jump point marker.");
			    }
			}
		  }
		...
		| TERRAIN_ID chance ':' region ',' FILLING ',' terrain_type
		  {
			...
			add_opvars(splev, "iii iiio",
				   (($4.x1 & 0xff) + (($4.y1 << 16) & 0xff)),
				   $4.x2, $4.y2,
				   (long)(3 + $<i>6), c, (long)$8.lit,
				   SPO_TERRAIN);
...
grave_detail	: GRAVE_ID ':' coord_or_var ',' string
		...
		| GRAVE_ID ':' coord_or_var ',' RANDOM_TYPE
		  {
			add_opvars(splev, "sio", NULL, 1, SPO_GRAVE);
		  }
		| GRAVE_ID ':' coord_or_var
		  {
			add_opvars(splev, "sio", NULL, 0, SPO_GRAVE);
		  }
		;
...
engraving_detail: ENGRAVING_ID ':' coord_or_var ',' engraving_type ',' string_or_var
		  {
			add_opvars(splev, "io", (long)$5, SPO_ENGRAVING);
		  }
		;
		...
		| VARSTRING
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_STRING, "string");
			add_opvars(splev, "v", $1);
		  }
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_STRING|SPOVAR_ARRAY, "string");
...
integer_or_var	: INTEGER
		...
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_INT|SPOVAR_ARRAY, "integer");
			add_opvars(splev, "iv", $3, $1);
		  }
		;
...
coord_or_var	: encodecoord
		...
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_COORD|SPOVAR_ARRAY, "coord");
			add_opvars(splev, "iv", $3, $1);
		  }
		;
...
region_or_var	: encoderegion
		...
		| VARSTRING
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_REGION, "region");
			add_opvars(splev, "v", $1);
		  }
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_REGION|SPOVAR_ARRAY, "region");
			add_opvars(splev, "iv", $3, $1);
		  }
		;
...
mapchar_or_var	: mapchar
		...
		| VARSTRING
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_MAPCHAR, "mapchar");
			add_opvars(splev, "v", $1);
		  }
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_MAPCHAR|SPOVAR_ARRAY, "mapchar");
			add_opvars(splev, "iv", $3, $1);
		  }
		;
...
monster_or_var	: encodemonster
		...
		| VARSTRING
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_MONST, "monster");
			add_opvars(splev, "v", $1);
		  }
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_MONST|SPOVAR_ARRAY, "monster");
			add_opvars(splev, "iv", $3, $1);
		  }
		;
...
object_or_var	: encodeobj
		...
		| VARSTRING '[' INTEGER ']'
		  {
			check_vardef_type(variable_definitions, $1,
					  SPOVAR_OBJ|SPOVAR_ARRAY, "object");
			add_opvars(splev, "iv", $3, $1);
		  }
		;
		...
		| comparestmt
		  {
			...
			if (n_if_list >= MAX_NESTED_IFS) {
			    lc_error("Comparison: Conditionals nested too deeply.");
			    n_if_list = MAX_NESTED_IFS - 1;
			}
...
/* This series of if statements is a hack for MSC 5.1.  It seems that its
   tiny little brain cannot compile if these are all one big if statement. */
			if ($3 <= 0 || $3 >= COLNO) {
			    lc_error("Region (%li,%li,%li,%li) out of level range "
				     "(x1)!", $3, $5, $7, $9);
			} else if ($5 < 0 || $5 >= ROWNO) {
			    lc_error("Region (%li,%li,%li,%li) out of level range "
				     "(y1)!", $3, $5, $7, $9);
...
/* This series of if statements is a hack for MSC 5.1.  It seems that its
   tiny little brain cannot compile if these are all one big if statement. */
			if ($2 < 0 || $2 > (int)max_x_map) {
			    lc_error("Region (%li,%li,%li,%li) out of map range "
				     "(x1)!", $2, $4, $6, $8);
