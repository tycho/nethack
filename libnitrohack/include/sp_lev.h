...
#define SPOVAR_COORD	0x04 /* coordinate, encoded in l;
				use SP_COORD_X() and SP_COORD_Y() */
#define SPOVAR_REGION	0x05 /* region, encoded in l; use SP_REGION_X1() etc */
#define SPOVAR_MAPCHAR	0x06 /* map char, in l */
#define SPOVAR_MONST	0x07 /* monster class & specific monster, encoded in l;
				use SP_MONST_... */
#define SPOVAR_OBJ	0x08 /* object class & specific object type, encoded in l;
				use SP_OBJ_... */
...
#define SP_REGION_PACK(x1,y1,x2,y2) (((x1) & 0xff) + (((y1) & 0xff) << 8) + \
				     (((x2) & 0xff) << 16) + (((y2) & 0xff) << 24))
...
typedef struct {
	...
	short female, invis, cancelled, revived, avenge,
	      fleeing, blinded, paralyzed, stunned, confused;
	long seentraps;
	short has_invent;
} monster;
